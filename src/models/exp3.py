...
class EXP3():
    ...
    def generate_results(self) -> None:
        ...
        for i, train_path in enumerate(train_files):
            ...
            y_train_list = list(X_train_df["LABELS"])
            y_test_list = list(X_test_df["LABELS"])
            
            X_train_df.drop("LABELS", axis=1, inplace=True)
            X_test_df.drop("LABELS", axis=1, inplace=True)

            # Convert to NumPy arrays for wider compatibility (especially for TabNet)
            X_train = X_train_df.to_numpy()
            X_test = X_test_df.to_numpy()
            y_train = np.array(y_train_list).reshape(-1) # Reshape y_train for TabNet
            y_test = np.array(y_test_list).reshape(-1) # Reshape y_train for TabNet
            # y_train_tabnet = y_train
            print(f"Distribution Train: {Counter(y_train)}, Test: {Counter(y_test)}")
            ...
            if pred_key not in PREDICTIONS:
                ...
                PROB_PREDICTIONS[pred_key][:,0] = y_test
            ...
            for j, model in enumerate(self.classification["MODEL"]):
                ...
                # print(type(prob_pred), type(pred), prob_pred, pred)
