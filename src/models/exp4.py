...
class EXP4():
    def __init__(self, args) -> None:
        ...
        self.temp_dir = os.path.join(args.data_dir, "temp/exp4")
        self.exp_result_dir = os.path.join(self.results_dir, "exp4")
        self.exp_models_dir = os.path.join(self.models_dir, "exp4")
        if self.drop_all:
            self.data_read_dir = os.path.join(args.data_dir, "temp_removed_all/data_345")            
            self.temp_dir = os.path.join(args.data_dir, "temp_removed_all/exp4")
            self.exp_result_dir = os.path.join(self.results_dir, "removed_all", "exp4")
            self.exp_models_dir = os.path.join(self.models_dir, "removed_all", "exp4")
        self.data_csv = os.path.join(self.data_dir, "target_ordinal_encoded_data.csv")
        self.result_excel = os.path.join(self.exp_result_dir, "EXP4_PERFORMANCE.xlsx")
    ...
    def generate_results(self) -> None:
        ...
        for i, train_path in enumerate(train_files):
            ...
            y_train_np = np.array(y_train).reshape(-1) # Reshape y_train for TabNet
            y_test_np = np.array(y_test).reshape(-1) # Reshape y_train for TabNet
            # y_train_tabnet = y_train_np
